...
# 06-forks needs an SDK zig can link against, so report it before anything
# consumes it. The fork builds aim xcrun at one themselves, so there is no
# machine change to offer. Repo path: nothing is stowed until 02-dotfiles.
